import React from "react";
import { FiSearch } from "react-icons/fi";

const Banner = ({ query, handleInputChange }) => {
	return (
		<div className="bg-white">
			<div className="max-w-screen-2xl container mx-auto xl:pr-24 pr-4 md:py-20 py-14">
				<h1 className="text-5xl font-bold text-primary mb-3">
					Find your <span className="text-indigo">new recipes </span>
					today
				</h1>
				<p className="text-lg text-black/70 mb-8">
					Savor the flavors with this incredible recipe! ...
				</p>

				<form>
					<div className="flex justify-start md:flex-row flex-col md:gap-0 gap-4">
						<div className="flex md:rounded-s-md rounded shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 md:w-1/2 w-full">
							<input
								type="text"
								id="title"
								name="title"
								value={query}
								onChange={handleInputChange}
								placeholder="What recipes are you looking for?"
								className="block flex-1 border-0 bg-transparent py-1.5 pl-8 text-gray-900 placeholder:text-gray-400 focus:right-0 sm:text-sm sm:leading-6 "
							/>
							<FiSearch className="absolute mt-2.5 ml-2 text-gray-400" />
						</div>
						<button className="bg-indigo-500 py-2 px-8 text-white md:rounded-s-none rounded">
							Search
						</button>
					</div>
				</form>
			</div>
		</div>
	);
};

export default Banner;
